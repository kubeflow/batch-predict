...
class TensorFlowModel(BaseModel):
    ...
    def __init__(self, client):
        ...
        if processor_cls:
            self._preprocess_fn = getattr(processor_cls, config.PREPROCESS_KEY, None)
            self._postprocess_fn = getattr(processor_cls, config.POSTPROCESS_KEY, None)
    ...
    def _get_columns(self, instances, stats, signature):
        ...
        """
        with stats.time(config.COLUMNARIZE_TIME):
            columns = utils.columnarize(instances)
    ...
    def postprocess(self, predicted_output, original_input=None, stats=None,
        ...
        """
        _, signature = self.get_signature(signature_name)
        with stats.time(config.ROWIFY_TIME):
        ...
        with stats.time(config.ENCODE_TIME):
            try:
                postprocessed_outputs = utils.encode_base64(
...
class SessionClient(TensorFlowClient):
    ...
    def predict(self, inputs, stats=None,
        ...
        stats[config.ENGINE] = "SessionRun"
        stats[config.FRAMEWORK] = config.TENSORFLOW_FRAMEWORK_NAME

        with stats.time(config.UNALIAS_TIME):
            _, signature = self.get_signature(signature_name)
        ...
        with stats.time(config.SESSION_RUN_TIME):
            try:
        ...
        with stats.time(config.ALIAS_TIME):
