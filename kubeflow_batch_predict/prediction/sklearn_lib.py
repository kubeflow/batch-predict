...
class SklearnClient(PredictionClient):
    ...
    def predict(self, inputs, stats=None, **kwargs):
        ...
        stats[config.FRAMEWORK] = config.SCIKIT_LEARN_FRAMEWORK_NAME
        stats[config.ENGINE] = config.SCIKIT_LEARN_FRAMEWORK_NAME
        try:
...
class SklearnModel(BaseModel):
    ...
    def __init__(self, client):
        ...
        if self._user_processor and hasattr(self._user_processor, config.PREPROCESS_KEY):
            self._preprocess = self._user_processor.preprocess
        else:
            self._preprocess = self._null_processor
        if self._user_processor and hasattr(self._user_processor, config.POSTPROCESS_KEY):
    ...
    def predict(self, instances, stats=None, **kwargs):
        ...
        kwargs.pop(config.SIGNATURE_KEY, None)
...
def create_sklearn_client(model_path, unused_tags):
    ...
    if not sklearn_predictor:
        error_msg = "Could not find either {} or {} in {}".format(
            config.MODEL_FILE_NAME_JOBLIB, config.MODEL_FILE_NAME_PICKLE, model_path)
