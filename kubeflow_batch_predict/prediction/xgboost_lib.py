...
from kubeflow_batch_predict.prediction import utils, config
from kubeflow_batch_predict.prediction._interfaces import PredictionClient
from kubeflow_batch_predict.prediction.base import PredictionError, _load_joblib_or_pickle_model
from kubeflow_batch_predict.prediction.sklearn_lib import SklearnModel
...
class XgboostClient(PredictionClient):
    ...
    def predict(self, inputs, stats=None, **kwargs):
        ...
        stats[config.FRAMEWORK] = config.XGBOOST_FRAMEWORK_NAME
        stats[config.ENGINE] = config.XGBOOST_FRAMEWORK_NAME
...
def create_xgboost_client(model_path, unused_tags):
    ...
    if not booster:
        error_msg = "Could not find {}, {}, or {} in {}".format(
            config.MODEL_FILE_NAME_JOBLIB, config.MODEL_FILE_NAME_PICKLE, config.MODEL_FILE_NAME_BST,
            model_path)
...
def _load_xgboost_model(model_path):
    ...
    import xgboost as xgb  # pylint: disable=g-import-not-at-top
    model_file = os.path.join(model_path, config.MODEL_FILE_NAME_BST)
    ...
    try:
        ...
        error_msg = "Could not load the model: {}. {}.".format(
            os.path.join(model_path, config.MODEL_FILE_NAME_BST), str(e))
        logging.critical(error_msg)
        raise PredictionError(PredictionError.FAILED_TO_LOAD_MODEL, error_msg)
