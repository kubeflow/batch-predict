...
from kubeflow_batch_predict.prediction import config
from kubeflow_batch_predict.prediction import utils
from kubeflow_batch_predict.prediction._interfaces import Model
...
class BaseModel(Model):
  ...
  """
    ...
    def predict(self, instances, stats=None, **kwargs):
        ...
        stats = stats or utils.Stats()
        self._validate_kwargs(kwargs)

        with stats.time(config.PREPROCESS_TIME):
            preprocessed = self.preprocess(instances, stats=stats, **kwargs)
        with stats.time(config.ENGINE_RUN_TIME):
            predicted_outputs = self._client.predict(
                preprocessed, stats=stats, **kwargs)
        with stats.time(config.POSTPROCESS_TIME):
...
def load_model_class(client, model_path):
  ...
  """
    model_class = load_custom_class(config.UserClassType.model_class)
    if not model_class:
        return None
...
def _new_processor_class(model_path=None):
    user_processor_cls = load_custom_class(config.UserClassType.processor_class)
    if user_processor_cls:
        user_preprocess_fn = getattr(user_processor_cls, config.PREPROCESS_KEY, None)
        user_postprocess_fn = getattr(user_processor_cls, config.POSTPROCESS_KEY, None)
        user_from_model_path_fn = getattr(user_processor_cls, config.FROM_MODEL_KEY, None)

        _validate_fn_signature(user_preprocess_fn,
                               ["self", "instances"],
                               config.PREPROCESS_KEY, user_processor_cls.__name__)
        _validate_fn_signature(user_postprocess_fn,
                               ["self", "instances"],
                               config.POSTPROCESS_KEY, user_processor_cls.__name__)
        _validate_fn_signature(user_from_model_path_fn, ["cls", "model_path"],
                               config.FROM_MODEL_KEY, user_processor_cls.__name__)
...
def _load_joblib_or_pickle_model(model_path):
  ...
  """
    ...
    try:
        if file_io.file_exists(os.path.join(model_path, config.MODEL_FILE_NAME_JOBLIB)):
            model_file_name = os.path.join(model_path, config.MODEL_FILE_NAME_JOBLIB)
            logging.info("Loading model %s using joblib.", model_file_name)
            with file_io.FileIO(model_file_name, mode="rb") as f:
                return joblib.load(f)

        elif file_io.file_exists(os.path.join(model_path, config.MODEL_FILE_NAME_PICKLE)):
            model_file_name = os.path.join(model_path, config.MODEL_FILE_NAME_PICKLE)
